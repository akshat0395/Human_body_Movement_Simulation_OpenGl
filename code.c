...
void display(void)
{
    ...
    glLoadName(TEXTID);
    glColor3f(1,1,0);
    DrawTextXY(-2.5,0.2,2.0,0.0015,"141207");
    DrawTextXY(-2.5,0.5,2.0,0.0015,"Akshat Mathur");
...
int main(int argc, char **argv)
{
   ...
   // start of glut windowing and control functions
    ...
    glutInitDisplayMode(GLUT_DOUBLE | GLUT_RGBA | GLUT_DEPTH);//sets the initial display mode
    ...
    glutInitWindowSize(1000, 1000);// Set the initial window size glutInitWindowSize(int width, int height);
    
    glutCreateWindow("Human Body Movement Simualtion");// creates a top-level window.
    
    myinit(); // The function is used to set the OpenGL state variables dealing with viewing and attributes.
